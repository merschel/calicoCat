...
preference.ode.f = @f_temp;

preference.numberOfSimulations = 1; %wenn 1 ist die density nicht richtig
preference.simulationTime = [0 1000];
preference.deltaT = .01;
preference.deltaX = .01;

preference.parallel = 1; 

preference.inic.interval = {{20 20};{40 40}};             % {{* *}} or {{* *};{* *};...}
preference.inic.distribution = {'uniformly'}; % 'normally', 'uniformly', 'integer' or 'list'

preference.para.interval = {{200 200}; {30 30};  {0.86 0.86};  {1.005 1.005};  {8 8};  {20 20}};              % {{* *}} or {{* *};{* *};...}eference.para.distribution = {'uniformly'}; % 'normally', 'uniformly' or 'integer' or 'list'
